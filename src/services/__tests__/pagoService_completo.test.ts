...
describe('pagoService - Cobertura Final', () => {
  ...
  it('getAll: debe manejar �xito y error', async () => {
  ...
  it('getByJudoka: debe manejar �xito', async () => {
    mockFrom.mockReturnValue(createQueryMock([{ id: '1' }]))
    const res = await pagoService.getByJudoka('j1')
    expect(res.success).toBe(true)
  })

  it('getByClub: debe manejar �xito y error', async () => {
    mockFrom.mockReturnValue(createQueryMock([{ id: '1' }]))
  ...
  it('getById: debe manejar �xito', async () => {
    mockFrom.mockReturnValue(createQueryMock({ id: '1' }))
    const res = await pagoService.getById('1')
    expect(res.success).toBe(true)
  })

  it('create: debe manejar errores espec�ficos de Postgres', async () => {
