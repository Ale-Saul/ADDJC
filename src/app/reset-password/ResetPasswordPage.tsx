...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      try {
        ...
        // Aquí solo verificamos si tenemos una sesión activa después de la redirección
        ...
        // Si no hay sesión pero hay un código, intentamos intercambiarlo solo si el servidor no lo hizo
        // Esto es un fallback, pero idealmente el servidor ya lo hizo
        ...
        if (hash && (hash.includes('type=recovery') || hash.includes('access_token'))) {
          console.log('Hash detectado, procesando...')
          ...
          // Intentar procesar el hash manualmente
        ...
        // Si no hay sesión y no hay hash, asumimos que estamos en el paso de solicitud
        setCheckingToken(false)
  ...
  const onResetRequest = async (data: { email: string }) => {
    ...
    try {
      // Usar el endpoint de callback para manejar el intercambio de código PKCE en el servidor
  ...
  const onResetPassword = async (data: any) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          // Usar window.location.href para forzar una recarga completa y asegurar que el middleware
          // detecte que no hay sesión activa
  ...
  const handleBackToLogin = async () => {
    try {
      // Cerrar cualquier sesión activa antes de volver al login
      const supabase = createClient()
      await supabase.auth.signOut()
      
      // Forzar recarga completa para limpiar cookies y estado
      window.location.href = '/login'
    } catch (error) {
  ...
  // Paso 2: Cambiar contraseña (cuando el usuario viene del email)
  return (
    <Box
      sx={{
        display: 'flex',
