...
export default function ArbitroEditPage({ params }: ArbitroEditPageProps) {
  ...
  const loadArbitro = useCallback(async () => {
    ...
    if (response.success && response.data) {
      ...
      setError(response.error || 'Error al cargar el ├írbitro')
    }
  ...
  const handleSuccess = () => {
    setSuccessMessage('├ürbitro actualizado exitosamente')
    loadArbitro()
  }
  ...
  const handleConfirmDeleteCertificacion = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setCertDeleteError(response.error || 'Error al eliminar la certificaci├│n')
        setCertToDelete(null)
      }
    } catch {
      setCertDeleteError('Error inesperado al eliminar la certificaci├│n')
      setCertToDelete(null)
    } finally {
    ...
    <>
      ...
      {arbitro && (
        <>
          ...
          >
            <DialogTitle>
              {certificacionEditando ? 'Editar Certificaci├│n' : 'Nueva Certificaci├│n'}
            </DialogTitle>
            <DialogContent>
              <CertificacionForm
          ...
          <ConfirmDialog
            ...
            message={certToDelete ? `┬┐Est├ís seguro de eliminar la certificaci├│n "${certToDelete.nombre_certificacion}"?` : ''}
