...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'RESET_NEW_DIRECTOR': return { ...state, newDirector: initialState.newDirector }
    ...
    try {
      ...
      if (!club && !directorTecnicoId && state.newDirector.nombres.trim() !== '') {
        ...
        if (!senseiResponse.success || !senseiResponse.data) {
          dispatch({ type: 'SET_ERROR', payload: senseiResponse.error || 'Error al crear el director técnico' })
          dispatch({ type: 'SET_LOADING', payload: false })
          return
        }
      ...
      if (response.success) {
        ...
        dispatch({ type: 'SET_ERROR', payload: response.error || 'Error al guardar el club' })
      }
    } catch (err) {
